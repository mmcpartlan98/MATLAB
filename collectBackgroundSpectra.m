...
[xScale, correctedSpectra, rawSpectra, ~] = dptRead(readPath, rowLength, colLength, 'n');



scoreArray = zeros(rowLength, colLength);
% Assume only one background to be loaded for now
[~, correctedBG, rawBG, ~] = dptRead(backgroundPath, 1, bgs, 'y');
...
for i = 2:numberOfSpectra
    ...
    isNew = 0;
    for e = 2:numberOfPreviousSpectra
...
if (prompt == 'y')
    ...
    if (prompt == 'y')
        ...
        for i = 2:numberOfSpectra
            ...
            prompt = input('Is this a background spectrum? (y/n): ', 's');
            if (isempty(prompt))
